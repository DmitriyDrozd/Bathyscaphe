...
function DepthBomb() {
    this.x = -STORAGE.terrains.x + STORAGE.rendWidth / 2;
    this.y = -STORAGE.terrains.y + STORAGE.rendHeight / 2;

    this.sprite = new PIXI.Sprite(DepthBomb.prototype.textures[0]);
    this.sprite.anchor.set(0.5, 0.5);
    this.sprite.position.set(this.x, this.y);
    this.dy = 3;
...
DepthBomb.prototype.textures = [];
DepthBomb.prototype.maxDistance = 600;

DepthBomb.prototype.updateSprite = function() {
    ++this.explodeStage;
    this.sprite.texture = DepthBomb.prototype.textures[this.explodeStage];
};
...
export { generateNextLevel, customMenuButton, getMenuBackground, Mine, Enemy,
    Torpedo, DepthBomb, keyboard, wallsCollision, hasCollided, mineInitializer,
    enemiesInitializer, createBubbles };
