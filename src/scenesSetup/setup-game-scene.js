...
function setupGameScene() {
    ...
    SUPPORT.DepthBomb.prototype.maxDistance = STORAGE.rendHeight * 1.5;
    STORAGE.torpedoes = [];
    STORAGE.depthBombs = [];
    ...
    const displacementTexture = STORAGE.loader.resources["../images/scene/displacement.png"].texture;
    displacementTexture.baseTexture.wrapMode = PIXI.WRAP_MODES.REPEAT;
    STORAGE.displacementSprite = new PIXI.Sprite(displacementTexture);

    STORAGE.background.addChild(bg);
    STORAGE.background.addChild(STORAGE.displacementSprite);
    const displacementFilter = new PIXI.filters.DisplacementFilter(STORAGE.displacementSprite);
    displacementFilter.width = bg.width;
    displacementFilter.height = bg.height;
    STORAGE.background.filters = [ STORAGE.filter, displacementFilter ];

    //mines
    if(!SUPPORT.Mine.prototype.textures.length) {
        const mineTextures = STORAGE.loader.resources["../images/trap/chainless-mine.png"].texture;
        for(let i = 0; i < 5; ++i) {
    ...
    //enemies
    if(!SUPPORT.Enemy.prototype.textures.length) {
        const shipsTextures = STORAGE.loader.resources["../images/ship/bathyscaphe-enemy.png"].texture;
        for(let i = 0; i < 3; ++i) {
            let y = i === 0 ? 4 : i * 90;
    ...
    //torpedoes
    if(!SUPPORT.Torpedo.prototype.textures.length) {
        let torpTexture;
        const torpTextures = STORAGE.loader.resources["../images/ammo/torpedo.png"].texture;
    ...
    //depth bomb
    if(!SUPPORT.DepthBomb.prototype.textures.length) {
        let depthBombTexture;
        const depthBombTextures = STORAGE.loader.resources["../images/ammo/depth-bomb.png"].texture;
        depthBombTexture = new PIXI.Texture(depthBombTextures, new PIXI.Rectangle(6, 68, 20, 34)); //0
        SUPPORT.DepthBomb.prototype.textures.push(depthBombTexture);
        depthBombTexture = new PIXI.Texture(depthBombTextures, new PIXI.Rectangle(32, 68, 30, 34)); //1
        SUPPORT.DepthBomb.prototype.textures.push(depthBombTexture);
        depthBombTexture = new PIXI.Texture(depthBombTextures, new PIXI.Rectangle(68, 56, 58, 46)); //2
        SUPPORT.DepthBomb.prototype.textures.push(depthBombTexture);
        depthBombTexture = new PIXI.Texture(depthBombTextures, new PIXI.Rectangle(135, 45, 38, 57)); //3
        SUPPORT.DepthBomb.prototype.textures.push(depthBombTexture);
        depthBombTexture = new PIXI.Texture(depthBombTextures, new PIXI.Rectangle(184, 35, 46, 67)); //4
        SUPPORT.DepthBomb.prototype.textures.push(depthBombTexture);
        depthBombTexture = new PIXI.Texture(depthBombTextures, new PIXI.Rectangle(238, 24, 54, 78)); //5
        SUPPORT.DepthBomb.prototype.textures.push(depthBombTexture);
        depthBombTexture = new PIXI.Texture(depthBombTextures, new PIXI.Rectangle(302, 18, 62, 84)); //6
        SUPPORT.DepthBomb.prototype.textures.push(depthBombTexture);
        depthBombTexture = new PIXI.Texture(depthBombTextures, new PIXI.Rectangle(374, 12, 72, 90)); //7
        SUPPORT.DepthBomb.prototype.textures.push(depthBombTexture);
        depthBombTexture = new PIXI.Texture(depthBombTextures, new PIXI.Rectangle(468, 6, 80, 108)); //8
        SUPPORT.DepthBomb.prototype.textures.push(depthBombTexture);
        depthBombTexture = new PIXI.Texture(depthBombTextures, new PIXI.Rectangle(560, 6, 55, 108)); //9
        SUPPORT.DepthBomb.prototype.textures.push(depthBombTexture);
        depthBombTexture = new PIXI.Texture(depthBombTextures, new PIXI.Rectangle(622, 6, 56, 108)); //10
        SUPPORT.DepthBomb.prototype.textures.push(depthBombTexture);
    }

    //player
    if(!STORAGE.playerTextures.length) {
        const playerTextures = STORAGE.loader.resources["../images/ship/bathyscaphe-player.png"].texture;
        for(let i = 0; i < 3; ++i) {
            let y = i === 0 ? 4 : i * 90;
